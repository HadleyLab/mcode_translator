...
class BenchmarkResult:
    ...
    def calculate_metrics(self, expected_entities: List[Dict[str, Any]] = None,
        ...
        framework.logger.debug(f"calculate_metrics called with expected_entities={expected_entities is not None}, expected_mappings={expected_mappings is not None}")
        ...
        framework.logger.debug(f"Extraction validation: expected_entities={expected_entities is not None}, extracted_entities={self.extracted_entities is not None}, len={len(self.extracted_entities) if self.extracted_entities else 0}")
        ...
        framework.logger.debug(f"Before extraction validation: expected_entities bool: {expected_entities is not None}")
        framework.logger.debug(f"Before extraction validation: self.extracted_entities bool: {self.extracted_entities is not None}")
        if expected_entities is not None and self.extracted_entities is not None:
            framework.logger.debug(f"Expected entities length: {len(expected_entities)}")
            framework.logger.debug(f"Extracted entities length: {len(self.extracted_entities)}")
            if len(expected_entities) > 0 and len(self.extracted_entities) > 0:
                ...
                framework.logger.debug(f"True positives: {true_positives_ext}, False positives: {false_positives_ext}, False negatives: {false_negatives_ext}")
                
                self.precision = true_positives_ext / (true_positives_ext + false_positives_ext) if (true_positives_ext + false_positives_ext) > 0 else None
            ...
            framework.logger.debug(f"One or both parameters are None: expected_entities={expected_entities is not None}, extracted_entities={self.extracted_entities is not None}")
            self.precision = None
            self.recall = None
            self.f1_score = None
        ...
        framework.logger.debug(f"Mapping validation: expected_mappings bool: {expected_mappings is not None}")
        ...
        if expected_mappings is not None and self.mcode_mappings is not None:
            ...
            if len(expected_mappings) > 0 and len(self.mcode_mappings) > 0:
                ...
                mapping_precision = true_positives_map / (true_positives_map + false_positives_map) if (true_positives_map + false_positives_map) > 0 else None
                mapping_recall = true_positives_map / (true_positives_map + false_negatives_map) if (true_positives_map + false_negatives_map) > 0 else None
    ...
    @staticmethod
    def _calculate_mcode_matches(actual_mappings: List[Dict[str, Any]],
        ...
        for i, actual_mapping in enumerate(actual_mappings):
            for j, expected_mapping in enumerate(expected_mappings):
                ...
                expected_rationale = expected_mapping.get('mapping_rationale', '').lower()
                
                # Exact match on all fields
                if (actual_element == expected_element and
                    actual_value == expected_value and
                    actual_source_idx == expected_source_idx and
                    abs(actual_confidence - expected_confidence) < 0.01 and
                    actual_rationale == expected_rationale):
        ...
        for i, actual_mapping in enumerate(actual_mappings):
            ...
            for j, expected_mapping in enumerate(expected_mappings):
                ...
                if actual_element == expected_element and actual_value == expected_value:
                    ...
                    matched_expected.add(j)
                    if debug and framework:
                        framework.logger.warning(f"   ✅ Clinical concept match: '{actual_element}'='{actual_value}' -> '{expected_element}'='{expected_value}'")
                    continue
        ...
        if debug and framework:
            framework.logger.warning(f"   Total actual mappings: {len(actual_mappings)}")
            framework.logger.warning(f"   Total expected mappings: {len(expected_mappings)}")
