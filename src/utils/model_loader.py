...
class ModelLoader:
    ...
    def get_model(self, model_key: str) -> ModelConfig:
        ...
        try:
            ...
            if not api_key_env_var:
                raise ValueError(
                    f"Configuration Error: 'api_key_env_var' not set for model '{model_key}'."
                )
    ...
    def get_default_model(self) -> None:
        ...
        """
        raise ValueError(
            "Default model fallbacks not allowed in strict implementation - model must be explicitly specified"
        )
