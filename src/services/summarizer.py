...
class McodeSummarizer:
    ...
    def _configure_detail_levels(self):
        ...
        self.element_configs = {
            ...
            'Procedure': {'priority': 10, 'template': "{subject}'s procedures (mCODE: Procedure{date_qualifier}) are {value}."},
            'MedicationRequest': {'priority': 11, 'template': "{subject}'s medications (mCODE: MedicationRequest) are {value}."},
            'SocialDeterminant': {'priority': 12, 'template': "{subject}'s social determinants (mCODE: SocialDeterminant) are {value}."},
            'Condition': {'priority': 13, 'template': "{subject}'s conditions (mCODE: Condition{date_qualifier}) are {value}."},
            'CauseOfDeath': {'priority': 14, 'template': "{subject}'s cause of death (mCODE: CauseOfDeath{date_qualifier}) is {value}{codes}."},

            # Trial elements
            'Trial': {'priority': 15, 'template': '{subject} is a Clinical Trial (mCODE: Trial).'},
            'TrialTitle': {'priority': 16, 'template': "{subject}'s title (mCODE: TrialTitle) is '{value}'."},
            'TrialStudyType': {'priority': 17, 'template': "{subject}'s study type (mCODE: TrialStudyType) is {value} study{codes}."},
            'TrialPhase': {'priority': 18, 'template': "{subject}'s phase (mCODE: TrialPhase) is {value}{codes}."},
            'TrialStatus': {'priority': 19, 'template': "{subject}'s status (mCODE: TrialStatus) is {value}{codes}."},
            'TrialLeadSponsor': {'priority': 20, 'template': "{subject}'s lead sponsor (mCODE: TrialLeadSponsor) is {value}{codes}."},
            'TrialCollaborators': {'priority': 21, 'template': "{subject}'s collaborators (mCODE: TrialCollaborators) include {value}."},
            'TrialInterventionModel': {'priority': 22, 'template': "{subject}'s intervention model (mCODE: TrialInterventionModel) is {value}{codes}."},
            'TrialPrimaryPurpose': {'priority': 23, 'template': "{subject}'s primary purpose (mCODE: TrialPrimaryPurpose) is {value}{codes}."},
            'TrialEnrollment': {'priority': 24, 'template': "{subject}'s enrollment (mCODE: TrialEnrollment) is {value} participants{codes}."},
            'TrialCancerConditions': {'priority': 25, 'template': "{subject}'s cancer conditions (mCODE: TrialCancerConditions) include {value}."},
            'TrialAgeCriteria': {'priority': 26, 'template': "{subject}'s age criteria (mCODE: TrialAgeCriteria) require patients {value}."},
            'TrialSexCriteria': {'priority': 27, 'template': "{subject}'s sex criteria (mCODE: TrialSexCriteria) require patients {value}."},
            'TrialHealthyVolunteers': {'priority': 28, 'template': "{subject}'s healthy volunteers criteria (mCODE: TrialHealthyVolunteers) {value}."},
            'TrialMedicationInterventions': {'priority': 29, 'template': "{subject}'s medication interventions (mCODE: TrialMedicationInterventions) include {value}."},
            'TrialOtherInterventions': {'priority': 30, 'template': "{subject}'s other interventions (mCODE: TrialOtherInterventions) include {value}."},
            'TrialPrimaryOutcomes': {'priority': 31, 'template': "{subject}'s primary outcomes (mCODE: TrialPrimaryOutcomes) include {value}."},
            'TrialStartDate': {'priority': 32, 'template': "{subject}'s start date (mCODE: TrialStartDate) is {value}{codes}."},
            'TrialCompletionDate': {'priority': 33, 'template': "{subject}'s completion date (mCODE: TrialCompletionDate) is {value}{codes}."},
        }
    ...
    def _extract_patient_elements(self, patient_data: Dict[str, Any], include_dates: bool) -> List[Dict[str, Any]]:
        ...
        if gender:
            elements.append({
                'element_name': 'Gender',
                'value': gender.lower(),
                'codes': 'SNOMED:248153007' if gender.lower() == 'male' else 'SNOMED:248152002',
                'date_qualifier': ''
            })
    ...
    def create_trial_summary(self, trial_data: Dict[str, Any]) -> str:
        ...
        nct_id = trial_data.get("protocolSection", {}).get("identificationModule", {}).get("nctId", "Unknown")
        if nct_id == "Unknown":
