...
class ClinicalNoteGenerator:
    ...
    def _generate_demographics_section(self, demographics: Dict[str, Any]) -> str:
        """Generate comprehensive demographics section."""
        demographics_info = []
        ...
        if demographics_info:
            if len(demographics_info) == 1:
                return f"Patient demographics: {demographics_info[0]}."
            else:
                return f"Patient demographics: {'; '.join(demographics_info[:-1])} and {demographics_info[-1]}."
        return ""

    def _generate_cancer_diagnosis_section(self, mcode_elements: Dict[str, Any]) -> str:
        """Generate cancer diagnosis section with dates."""
        cancer_diagnoses = []
        ...
        if cancer_diagnoses:
            if len(cancer_diagnoses) == 1:
                return f"Patient has cancer diagnosis: {cancer_diagnoses[0]}."
            else:
                return f"Patient has cancer diagnoses: {'; '.join(cancer_diagnoses[:-1])} and {cancer_diagnoses[-1]}."
        return ""

    def _generate_biomarker_section(self, mcode_elements: Dict[str, Any]) -> str:
        """Generate biomarker results section."""
        biomarkers = []
        ...
        if biomarkers:
            if len(biomarkers) == 1:
                return f"Patient biomarker profile includes: {biomarkers[0]}."
            else:
                return f"Patient biomarker profile includes: {'; '.join(biomarkers[:-1])} and {biomarkers[-1]}."
        return ""

    def _generate_staging_section(self, mcode_elements: Dict[str, Any]) -> str:
        """Generate cancer staging section."""
        staging = []
        ...
        if staging:
            if len(staging) == 1:
                return f"Patient cancer staging: {staging[0]}."
            else:
                return f"Patient cancer staging: {'; '.join(staging[:-1])} and {staging[-1]}."
        return ""

    def _generate_treatments_section(self, mcode_elements: Dict[str, Any]) -> str:
        """Generate cancer treatments and procedures section with dates."""
        treatments = []
        ...
        if treatments:
            if len(treatments) == 1:
                return f"Patient cancer treatments include: {treatments[0]}."
            else:
                return f"Patient cancer treatments include: {'; '.join(treatments[:-1])} and {treatments[-1]}."
        return ""

    def _generate_genetics_section(self, mcode_elements: Dict[str, Any]) -> str:
        """Generate genetic information section."""
        genetics = []
    ...
    def _generate_performance_section(self, mcode_elements: Dict[str, Any]) -> str:
        ...
        if performance_status:
            if len(performance_status) == 1:
                return f"Patient performance status: {performance_status[0]}."
            else:
                return f"Patient performance status: {'; '.join(performance_status[:-1])} and {performance_status[-1]}."
        return ""

    def _generate_vitals_section(self, mcode_elements: Dict[str, Any]) -> str:
        """Generate vital signs and measurements section."""
        vital_signs = []
        ...
        if vital_signs:
            if len(vital_signs) == 1:
                return f"Patient vital signs: {vital_signs[0]}."
            else:
                return f"Patient vital signs: {'; '.join(vital_signs[:-1])} and {vital_signs[-1]}."
        return ""

    def _generate_lab_section(self, mcode_elements: Dict[str, Any]) -> str:
        """Generate laboratory results section."""
        lab_results = []
        ...
        if lab_results:
            if len(lab_results) == 1:
                return f"Patient laboratory results: {lab_results[0]}."
            else:
                return f"Patient laboratory results: {'; '.join(lab_results[:-1])} and {lab_results[-1]}."
        return ""

    def _generate_comorbidities_section(self, mcode_elements: Dict[str, Any]) -> str:
        """Generate comorbidities and other conditions section."""
        comorbidities = []
        ...
        if comorbidities:
            if len(comorbidities) == 1:
                return f"Patient comorbidities: {comorbidities[0]}."
            else:
                return f"Patient comorbidities: {'; '.join(comorbidities[:-1])} and {comorbidities[-1]}."
        return ""

    def _generate_allergies_section(self, mcode_elements: Dict[str, Any]) -> str:
        ...
        if allergies:
            if len(allergies) == 1:
                return f"Patient allergies: {allergies[0]}."
            else:
                return f"Patient allergies: {'; '.join(allergies[:-1])} and {allergies[-1]}."
        return ""

    def _generate_immunization_section(self, mcode_elements: Dict[str, Any]) -> str:
        """Generate immunization history section."""
        immunizations = []
        ...
        if immunizations:
            if len(immunizations) == 1:
                return f"Patient immunization history: {immunizations[0]}."
            else:
                return f"Patient immunization history: {'; '.join(immunizations[:-1])} and {immunizations[-1]}."
        return ""

    def _generate_family_history_section(self, mcode_elements: Dict[str, Any]) -> str:
        """Generate family history section."""
        family_history = []
        ...
        if family_history:
            if len(family_history) == 1:
                return f"Patient family history: {family_history[0]}."
            else:
