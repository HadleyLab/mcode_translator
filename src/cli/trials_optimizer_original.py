...
class CrossValidationSuite:
    ...
    async def initialize(self, max_workers: int = 3) -> None:
        ...
        self.logger.info(
            f"🚀 Initializing optimization suite with {max_workers} workers"
        )
    ...
    def get_available_prompts(self) -> List[str]:
        ...
        self.logger.info(
            f"📋 Found {len(prompt_names)} available prompts: {prompt_names}"
        )
        return prompt_names
    ...
    def load_trials(self, trials_file: str) -> List[Dict[str, Any]]:
        ...
        if isinstance(data, dict) and "successful_trials" in data:
            ...
            raise ValueError(
                "Invalid trials file format. Expected a JSON object with a 'successful_trials' key or a list of trials."
            )
    ...
    def generate_combinations(
        self, prompts: List[str], models: List[str], max_combinations: int = None
    ) -> List[Tuple[str, str]]:
        ...
        if max_combinations and len(combinations) > max_combinations:
            ...
            self.logger.info(
                f"🎯 Limited to {len(combinations)} combinations (sampling)"
            )

        self.logger.info(
            f"🔄 Generated {len(combinations)} prompt × model combinations"
        )
        return combinations
    ...
    async def run_optimization(
        ...
        for prompt_name, model_name in combinations:
            for i, trial in enumerate(trials):
                ...
                gold_standard_mappings = trial.get("McodeResults", {}).get(
                    "mcode_mappings", []
                )
                ...
                task_id = await self.task_queue.add_task(
                    task, task_callbacks[task.task_id]
                )
        ...
        await self._wait_for_completion_with_progress(
            len(tasks_created), len(combinations)
        )
    ...
    async def _wait_for_completion_with_progress(
        self, total_tasks: int, total_combinations: int
    ) -> None:
        ...
        while True:
            ...
            if (
                current_time - last_check_time >= 30
            ):  # Every 30 seconds, recalculate rate
            ...
            if rate_per_hour > 0:
                ...
                eta_str = (
                    f"{eta_hours*60:.0f}m" if eta_hours < 1 else f"{eta_hours:.1f}h"
                )
            else:
                eta_str = "calculating..."
    ...
    def _process_combination_results(
        self,
        combinations: List[Tuple[str, str]],
        trials: List[Dict[str, Any]],
        total_tasks: int,
        ...
        for task_id, result_data in self.task_results.items():
            ...
            if task.status == TaskStatus.SUCCESS and task.result:
                ...
                quality_score = task.result.validation_results.get(
                ...
                self.logger.info(
                    f"  ✅ {prompt_name} × {model_name} - Trial {trial_index+1}: {num_mappings} mappings, quality {quality_score:.3f}, f1 {f1_score:.3f}"
                )
            else:
                trial_result = {
                    "trial_id": trial_id,
                ...
                self.logger.error(
                    f"  ❌ {prompt_name} × {model_name} - Trial {trial_index+1} failed: {task.error_message}"
                )
        ...
        for combo_key, combo_data in combination_results.items():
            ...
            combination_result = {
                ...
                "success_rate": (
                    successful_trials / num_trials if num_trials > 0 else 0.0
                ),
                "total_mappings": combo_data["total_mappings"],
                "total_entities": combo_data["total_entities"],
                "avg_mappings_per_trial": (
                    combo_data["total_mappings"] / num_trials if num_trials > 0 else 0
                ),
                "avg_entities_per_trial": (
                    combo_data["total_entities"] / num_trials if num_trials > 0 else 0
                ),
                "avg_quality_score": (
            ...
            self.logger.info(
                f"📊 Combination complete: {combo_data['prompt_name']} × {combo_data['model_name']} - "
    ...
    def analyze_results(self) -> Dict[str, Any]:
        ...
        overall_success_rate = (
            total_successful / total_trials if total_trials > 0 else 0
        )

        # Quality metrics
        quality_scores = [
            r["avg_quality_score"] for r in self.results if r["avg_quality_score"] > 0
        ]
        ...
        avg_mappings_per_combo = (
            total_mappings / total_combinations if total_combinations > 0 else 0
        )
        ...
        analysis = {
            ...
            "prompt_analysis": {
                prompt: {
                    "avg_quality": sum(stats["quality_scores"])
                    / len(stats["quality_scores"]),
                    "avg_mappings": sum(stats["mappings"]) / len(stats["mappings"]),
                    "avg_success_rate": sum(stats["success_rates"])
                    / len(stats["success_rates"]),
    ...
    def generate_detailed_report(self, timestamp: str) -> None:
        ...
        with open(report_file, "w") as f:
            ...
            f.write("## Summary\n\n")
            f.write(
                f"- **Total Combinations Tested**: {summary.get('total_combinations', 0)}\n"
            )
            f.write(
                f"- **Overall Success Rate**: {summary.get('overall_success_rate', 0):.1%}\n"
            )
            f.write(
                f"- **Average Quality Score**: {summary.get('average_quality_score', 0):.3f}\n"
            )
            f.write(
                f"- **Average F1-Score**: {summary.get('average_f1_score', 0):.3f}\n"
            )
            f.write(
                f"- **Total mCODE Mappings**: {summary.get('total_mappings_generated', 0)}\n\n"
            )
            ...
            f.write(
                f"- **Highest Quality**: {best.get('highest_quality', {}).get('combination', 'N/A')} "
                f"({best.get('highest_quality', {}).get('quality_score', 0):.3f})\n"
            )
            f.write(
                f"- **Highest F1-Score**: {best.get('highest_f1_score', {}).get('combination', 'N/A')} "
                f"({best.get('highest_f1_score', {}).get('f1_score', 0):.3f})\n"
            )
            f.write(
                f"- **Most Mappings**: {best.get('most_mappings', {}).get('combination', 'N/A')} "
                f"({best.get('most_mappings', {}).get('total_mappings', 0)} mappings)\n"
            )
            f.write(
                f"- **Most Reliable**: {best.get('most_reliable', {}).get('combination', 'N/A')} "
            ...
            f.write(
                "| Prompt | Avg Quality | Avg F1-Score | Avg Mappings | Success Rate |\n"
            )
            f.write(
                "|--------|-------------|--------------|--------------|---------------|\n"
            )
            for prompt, stats in prompt_analysis.items():
                f.write(
                    f"| {prompt} | {stats['avg_quality']:.3f} | {stats['avg_f1_score']:.3f} | {stats['avg_mappings']:.1f} | {stats['avg_success_rate']:.1%} |\n"
                )
            f.write("\n")
            ...
            f.write(
                "| Model | Avg Quality | Avg F1-Score | Avg Mappings | Success Rate |\n"
            )
            f.write(
                "|-------|-------------|--------------|--------------|---------------|\n"
            )
            for model, stats in model_analysis.items():
                f.write(
                    f"| {model} | {stats['avg_quality']:.3f} | {stats['avg_f1_score']:.3f} | {stats['avg_mappings']:.1f} | {stats['avg_success_rate']:.1%} |\n"
                )
...
async def main():
    ...
    if args.pairwise and not PAIRWISE_AVAILABLE:
        logger.error(
            "❌ Pairwise cross-validation not available. Please check the implementation."
        )
        return
    ...
    try:
        ...
        if args.pairwise:
            ...
            combinations = suite.generate_combinations(
                prompts, models, args.max_combinations
            )

            logger.info(
                f"🎯 Testing {len(combinations)} combinations across {len(trials)} trials"
            )
            ...
            logger.info(
                f"✅ Overall success rate: {analysis['summary']['overall_success_rate']:.1%}"
            )
            logger.info(
                f"🎯 Average quality score: {analysis['summary']['average_quality_score']:.3f}"
            )
            logger.info(f" F1-Score: {analysis['summary']['average_f1_score']:.3f}")
            logger.info(
                f"📋 Total mCODE mappings: {analysis['summary']['total_mappings_generated']}"
            )

            # Show best performer
            best = analysis["best_performers"]["highest_f1_score"]
            logger.info(
                f"🏆 Best combination (F1-Score): {best['combination']} (f1_score: {best['f1_score']:.3f})"
            )
