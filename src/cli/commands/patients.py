...
def patients_pipeline(
    ...
    archive_path: Optional[str] = typer.Option(None, help="Path or identifier for patient data archive"),
    ...
    # Process flags
    process: bool = typer.Option(False, "--process", help="Process patient data with mCODE mapping"),
    input_file: Optional[str] = typer.Option(None, help="Path to patient data file (NDJSON format) for processing"),
    ...
    summary_input_file: Optional[str] = typer.Option(None, help="Path to processed patient data file for summarization"),
    ...
    try:
        ...
        if not any([fetch, process, summarize]):
            console.print("[red]❌ Must specify at least one pipeline stage: --fetch, --process, or --summarize[/red]")
            raise typer.Exit(1)
        ...
        if fetch:
            ...
            if fetch_result.success:
                console.print(f"[green]✅ Fetched {len(fetch_result.data) if fetch_result.data else 0} patients[/green]")
        ...
        if process:
            ...
            if process_result.success:
                ...
                if process_result.metadata:
                    ...
                    console.print(f"[green]📊 Processed {successful}/{total} patients successfully[/green]")
        ...
        if summarize:
            ...
            if process and pipeline_results["process"]:
                ...
                console.print("[red]❌ Either --process or --summary-input-file required for --summarize[/red]")
                raise typer.Exit(1)
            ...
            if summarize_result.success:
                ...
                if summarize_result.metadata:
                    ...
                    console.print(f"[green]📊 Summarized {successful}/{total} patients successfully[/green]")
        ...
        console.print("[bold green]🎉 Patient pipeline execution completed successfully![/bold green]")
