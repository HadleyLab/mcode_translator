...
class ConcurrentFetcher:
    ...
    async def initialize(self) -> None:
        ...
        logger.info(f"🚀 Initializing concurrent fetcher with {self.config.max_workers} workers")
        self.task_queue = await initialize_task_queue(max_workers=self.config.max_workers)
    ...
    async def process_trial_list(
        ...
        try:
            ...
            logger.info(f"📋 Successfully fetched {len(trials)} trials, processing concurrently")
            ...
            raise ClinicalTrialsAPIError(f"Concurrent trial processing failed: {str(e)}")
    ...
    async def _process_trials_concurrently(
        ...
        logger.info(f"   📊 Success rate: {len(successful_results)/len(trials)*100:.1f}%")
    ...
    async def _wait_for_completion_with_progress(self, total_tasks: int) -> None:
        ...
        logger.info(f"🏁 All tasks completed in {final_elapsed:.2f}s "
