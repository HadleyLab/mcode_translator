...
class McodeMapper(LlmBase, Loggable):
    ...
    def map_to_mcode(
        ...
        try:
            ...
            mapped_elements = self._parse_llm_response(
                parsed_response, source_references
            )
    ...
    def _validate_entities(
        self, entities: List[Dict[str, Any]], clinical_text: str = None
    ) -> None:
    ...
    def _call_llm_mapping(
        self, prompt: str, prompt_key: str = "generic_mapping"
            ...
            for element in transformed_elements:
                ...
                if validation_result.valid:
                    ...
                    if source_references:
                        self._connect_source_references_strict(
                            element, source_references
                        )

                    validated_elements.append(element)
                else:
                    raise McodeMappingError(
                        f"Invalid mCODE element: {validation_result.errors}"
                    )
    ...
    def _validate_mapping_response_structure(
        ...
        for mapping in parsed_response["mcode_mappings"]:
            if "mcode_element" not in mapping:
                raise LlmResponseError(
                    "Each mapping must contain 'mcode_element' field"
                )

    def _validate_mcode_element_strict(
        self, element: Dict[str, Any]
    ) -> McodeValidationResult:
    ...
    def _connect_source_references_strict(
        self, element: Dict[str, Any], source_references: List[SourceReference]
    ) -> None:
        """
        ...
        if "source_text_fragment" not in element:
            raise McodeMappingError(
                "mCODE element missing required source_text_fragment field for reference connection"
            )
        ...
        if not llm_text_fragment:
            raise McodeMappingError(
                "Empty source_text_fragment in mCODE element - cannot connect source references"
            )
        ...
        for ref in source_references:
            ...
            if (
                llm_text_fragment.lower() in ref.text_fragment.lower()
                or ref.text_fragment.lower() in llm_text_fragment.lower()
            ):
    ...
    def _validate_mcode_compliance_strict(
        ...
        validation_results["compliance_score"] = (
            compliant_count / total_elements if total_elements > 0 else 0.0
        )
