"""mCODE Translation Optimizer - Working Reactive Implementation"""
...
class McodeOptimizerWorking:
    """Working mCODE optimizer with proper UI updates"""
    ...
    def setup_ui(self):
        """Setup main UI"""
        ui.label('🎯 mCODE Optimization - Working Implementation').classes('text-3xl font-bold text-center mb-6')
        ...
        with ui.card().classes('w-full mb-4'):
            ...
            with ui.row().classes('gap-4 items-center'):
                ...
                self.start_button = ui.button(
                    '🚀 Start Optimization', 
                    on_click=self._start_optimization_click
    ...
    def _update_stats_display(self):
        ...
        with self.stats_container:
            ...
            colors = {'Queued': 'gray', 'Processing': 'orange', 'Success': 'green', 'Failed': 'red'}
            for status, count in status_counts.items():
                if count > 0:
                    ui.chip(f'{status}: {count}', color=colors[status])
    ...
    def _start_optimization_click(self):
        """Handle start button click - run optimization in background"""
        self.start_button.set_enabled(False)
        self.stop_button.set_enabled(True)
        self.is_running = True
        
        # Show notification in main thread before starting background task
        queued_count = len([t for t in self.tasks.values() if t['status'] == 'Queued'])
        ui.notify(f'Starting optimization of {queued_count} tasks with {self.max_concurrent_tasks} concurrent workers...')
        
        # Start background task
        background_tasks.create(self._run_optimization())
    
    async def _run_optimization(self):
        """Run optimization in background with UI updates"""
        try:
            ...
            # Log error instead of using ui.notify in background task
    ...
    async def _process_single_task(self, task):
        ...
        try:
            ...
            self._refresh_ui()  # Update UI immediately
...
def main():
    ...
    optimizer = McodeOptimizerWorking()
    optimizer.setup_ui()


if __name__ in {"__main__", "__mp_main__"}:
    main()
    ui.run(title="mCODE Optimizer - Working", port=8083)