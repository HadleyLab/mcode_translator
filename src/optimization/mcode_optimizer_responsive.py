"""mCODE Translation Optimizer - Truly Responsive Implementation"""
...
class McodeOptimizerResponsive:
    """Truly responsive mCODE optimizer using reactive state"""
    
    def __init__(self):
        """Initialize optimizer"""
        # Reactive state
        self.tasks = []  # List of TaskState objects for reactivity
        self.task_lookup = {}  # Dict for fast lookup
        ...
        for model_key, model_config in self.models.items():
            for prompt_info in self.prompts:
                for trial_id in self.trials.keys():
                    ...
                    self.task_lookup[str(task_id)] = task
                    task_id += 1
    ...
    def _update_stats(self):
        ...
        self.stats['queued'] = counts['Queued']
        self.stats['processing'] = counts['Processing'] 
        self.stats['success'] = counts['Success']
        self.stats['failed'] = counts['Failed']
        
        # Performance stats
        success_tasks = [t for t in self.tasks if t.status == 'Success' and t.f1_score is not None]
        if success_tasks:
            self.stats['avg_f1'] = sum(t.f1_score for t in success_tasks) / len(success_tasks)
            self.stats['best_f1'] = max(t.f1_score for t in success_tasks)
        else:
            self.stats['avg_f1'] = 0.0
            self.stats['best_f1'] = 0.0
    
    def setup_ui(self):
        """Setup main UI with true reactivity"""
        ui.label('🎯 mCODE Optimization - Truly Responsive').classes('text-3xl font-bold text-center mb-6')
        ...
        # Stats section with reactive bindings
        with ui.card().classes('w-full mb-4'):
            ui.label('📊 Statistics').classes('text-xl font-bold mb-4')
            
            with ui.row().classes('gap-4') as stats_row:
                self.stats_labels = {
                    'queued': ui.chip().bind_text_from(self, 'stats', backward=lambda s: f"Queued: {s['queued']}").props('color=gray'),
                    'processing': ui.chip().bind_text_from(self, 'stats', backward=lambda s: f"Processing: {s['processing']}").props('color=orange'),
                    'success': ui.chip().bind_text_from(self, 'stats', backward=lambda s: f"Success: {s['success']}").props('color=green'),
                    'failed': ui.chip().bind_text_from(self, 'stats', backward=lambda s: f"Failed: {s['failed']}").props('color=red'),
                    'avg_f1': ui.chip().bind_text_from(self, 'stats', backward=lambda s: f"Avg F1: {s['avg_f1']:.3f}").props('color=blue'),
                    'best_f1': ui.chip().bind_text_from(self, 'stats', backward=lambda s: f"Best F1: {s['best_f1']:.3f}").props('color=blue')
                }
        ...
        with ui.card().classes('w-full'):
            ...
            # Create table with reactive content
            with ui.scroll_area().classes('w-full h-[600px]'):
                # Table container that updates reactively
                self.table_container = ui.column().classes('w-full')
                
                # Initial render
                self._render_table()
                
                # Set up periodic refresh for true reactivity
                ui.timer(1.0, self._periodic_update)
    ...
    def _render_task_row(self, task: TaskState):
        ...
        with ui.row().classes('w-full p-2 text-xs border-b items-center hover:bg-gray-50'):
            ...
            # Elements count
            elem_count = len(task.extracted_elements)
    ...
    def _show_task_details(self, task: TaskState):
    ...
    def _periodic_update(self):
        """Periodic update for responsive UI"""
        if self.is_running:
            self._update_stats()
            self._render_table()
    ...
    async def _process_single_task(self, task: TaskState):
        ...
        try:
            ...
            # Get trial data
            trial_data = self.trials[task.trial]
            ...
            if result and result.get('status') == 'success':
                ...
                if 'expected_elements' in trial_data:
                    expected = set(trial_data['expected_elements'])
...
def main():
    ...
    optimizer = McodeOptimizerResponsive()
    optimizer.setup_ui()


if __name__ in {"__main__", "__mp_main__"}:
    main()
    ui.run(title="mCODE Optimizer - Truly Responsive", port=8084)