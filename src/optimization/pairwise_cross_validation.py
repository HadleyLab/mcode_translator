...
from src.pipeline.task_queue import PipelineTaskQueue, BenchmarkTask, initialize_task_queue, shutdown_task_queue
...
class PairwiseCrossValidator:
    ...
    async def initialize(self, max_workers: int = 3) -> None:
        ...
        self.logger.info(f"🚀 Initializing pairwise validator with {max_workers} workers")
    ...
    def generate_pairwise_tasks(
        ...
        for i, (gold_prompt, gold_model, gold_trial_id, gold_trial) in enumerate(all_combinations):
            for j, (comp_prompt, comp_model, comp_trial_id, comp_trial) in enumerate(all_combinations):
    ...
    async def run_pairwise_validation(
        self,
        tasks: List[PairwiseComparisonTask],
        max_workers: int = 3
    ) -> None:
        ...
        for task in tasks:
            try:
                ...
                if processed_tasks % 10 == 0:
                    ...
                    self.logger.info(f"📊 Progress: {processed_tasks}/{total_tasks} ({progress:.1f}%)")
        ...
        try:
            ...
            if gold_key in self.combination_cache:
                ...
                gold_pipeline = McodePipeline(
                    prompt_name=task.gold_prompt,
                    model_name=task.gold_model
                )
            ...
            if comp_key in self.combination_cache:
                ...
                comp_pipeline = McodePipeline(
                    prompt_name=task.comp_prompt,
                    model_name=task.comp_model
                )
                pipeline_result = await asyncio.to_thread(
    ...
    def _calculate_comparison_metrics(self, task: PairwiseComparisonTask) -> None:
        ...
        task.comparison_metrics = {
            **mapping_metrics,
            ...
            'gold_compliance_score': task.gold_result.validation_results.get('compliance_score', 0),
            'comp_compliance_score': task.comp_result.validation_results.get('compliance_score', 0)
        }
        
    def _calculate_mapping_metrics(self, gold_mappings: List[Dict], comp_mappings: List[Dict]) -> Dict[str, Any]:
        """Calculate mapping-level comparison metrics with detailed examples."""
        # Convert mappings to comparable strings
        gold_strings = [f"{m.get('mcode_element', '')}={json.dumps(m.get('value', ''))}" for m in gold_mappings]
        comp_strings = [f"{m.get('mcode_element', '')}={json.dumps(m.get('value', ''))}" for m in comp_mappings]
    ...
    def analyze_pairwise_results(self) -> Dict[str, Any]:
        ...
        analysis = {
            'summary': {
                ...
                'success_rate': len(successful_results) / len(self.pairwise_results) if self.pairwise_results else 0,
        ...
        with open(results_file, 'w') as f:
            json.dump({
                'metadata': {
                    ...
                    'successful_tasks': len([r for r in self.pairwise_results if r.status == TaskStatus.SUCCESS])
                },
                'results': [{
                    'task_id': r.task_id,
                    'trial_id': r.trial_id,
        ...
        for result in self.pairwise_results:
            if result.status == TaskStatus.SUCCESS:
                detailed_examples.append({
                    ...
                    'true_positive_examples': result.comparison_metrics.get('true_positive_examples', []),
                    'false_positive_examples': result.comparison_metrics.get('false_positive_examples', []),
                    'false_negative_examples': result.comparison_metrics.get('false_negative_examples', [])
                })
    ...
    def generate_detailed_report(self, timestamp: str) -> None:
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"- **Unique Configuration Pairs**: {summary.get('unique_config_pairs', 0)}\n\n")
            ...
            for metric_name, stats in overall.items():
                f.write(f"| {metric_name} | {stats['mean']:.3f} | {stats['median']:.3f} | {stats['stdev']:.3f} | {stats['min']:.3f} | {stats['max']:.3f} |\n")
        ...
        if overall_metrics:
            ...
            for metric_name, stats in overall_metrics.items():
                if metric_name in ['mapping_f1_score', 'mapping_jaccard_similarity']:
                    self.logger.info(f"   {metric_name.replace('_', ' ').title()}: {stats['mean']:.3f} (mean)")
