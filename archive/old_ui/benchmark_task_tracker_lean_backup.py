        ...
        try:
            ...
            try:
                ...
                if not response or not response.strip():
                    raise ValueError("STRICT: Empty response from LLM API call")
                
                mock_response = response.strip()
            except Exception as e:
                error_msg = f"STRICT LLM CALL FAILED: {str(e)}"
                print(error_msg)
                raise Exception(error_msg)  # Force debugging
    ...
    async def _process_tasks(self, task_ids: List[str]):
        ...
        async def process_single_task(task_id: str, worker_num: int):
            async with semaphore:
                ...
                # STRICT: Execute real validation using optimization framework
                try:
                    from src.optimization.prompt_optimization_framework import PromptOptimizationFramework
                    
                    # Get real framework instance
                    framework = PromptOptimizationFramework()
                    
                    # Execute real benchmark
                    benchmark_result = await framework.run_benchmark_async(
                        prompt_variant_id=validation['prompt'],
                        api_config_name=validation['model'], 
                        test_case_id=validation['trial'],
                        pipeline_type=validation['pipeline_type']
                    )
                    
                    if not benchmark_result:
                        raise ValueError(f"STRICT: No benchmark result returned for task {task_id}")
                    
                    # STRICT: Extract real metrics or fail immediately
                    if not hasattr(benchmark_result, 'f1_score') or benchmark_result.f1_score is None:
                        raise ValueError(f"STRICT: Benchmark result missing f1_score for task {task_id}")
                    
                    validation['status'] = 'Success'
                    validation['f1_score'] = benchmark_result.f1_score
                    validation['precision'] = benchmark_result.precision
                    validation['recall'] = benchmark_result.recall
                    validation['duration_ms'] = benchmark_result.execution_time_ms
                    validation['input_tokens'] = benchmark_result.input_tokens
                    validation['output_tokens'] = benchmark_result.output_tokens
                    validation['total_tokens'] = benchmark_result.total_tokens
                    validation['cost_usd'] = benchmark_result.cost_usd
                    validation['quality_score'] = benchmark_result.f1_score
                    
                except Exception as e:
                    # STRICT: Fail immediately with detailed error
                    error_msg = f"STRICT VALIDATION FAILED - Task: {task_id}, Error: {str(e)}"
                    print(error_msg)
                    validation['status'] = 'Failed'
                    validation['details'] = error_msg
                    raise Exception(error_msg)  # Propagate to force debugging
