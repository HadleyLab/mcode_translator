...
class CodeExtractionModule(Loggable):
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.logger.info("Code Extraction Module initialized")
    ...
    def _process_text(self, input_text: str, entities_str: str) -> Dict[str, Any]:
        ...
        Args:
            ...
            entities_str: JSON string representation of pre-identified entities
            
        Returns:
            Dictionary containing extracted codes and metadata, or error information
        """
        # Parse entities string back to list
        import json
        entities = json.loads(entities_str) if entities_str else None
        ...
        try:
            ...
            try:
                identified_codes = self.identify_all_codes(input_text)
            except Exception as e:
                self.logger.error(f"Code identification failed: {str(e)}")
                raise RuntimeError(f"Code identification failed: {str(e)}")
            ...
            for system, codes in identified_codes.items():
                ...
                for code in codes:
                    try:
                        ...
                        raise RuntimeError(f"Validation failed for code {code['code']}: {str(e)}")
            ...
            if entities:
                try:
                    ...
                    raise RuntimeError(f"Entity code extraction failed: {str(e)}")
            ...
            raise RuntimeError(f"Criteria processing failed: {str(e)}")
...
#     result = code_extractor.process_text(sample_text)
#
