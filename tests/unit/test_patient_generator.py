...
def sample_ndjson_content(sample_fhir_bundle):
    ...
    bundle2["entry"][0]["resource"]["name"] = bundle2["entry"][0]["resource"]["name"].copy()
    bundle2["entry"][0]["resource"]["name"][0] = bundle2["entry"][0]["resource"]["name"][0].copy()
...
@pytest.fixture
def create_test_zip(tmp_path, sample_fhir_bundle, sample_single_patient, sample_ndjson_content):
...
def test_patient_generator_basic_loading(create_test_zip):
    ...
    assert len(generator) == 4  # 1 bundle + 1 single patient + 2 from NDJSON + 1 invalid file that gets parsed
    ...
    for patient_bundle in patients:
        patient_entry = next(
            (e for e in patient_bundle.get('entry', [])
...
def test_patient_generator_exclude_ids(create_test_zip):
    ...
    patient_id = next(
        (e.get('resource', {}).get('id') for e in random_patient.get('entry', [])
         if e.get('resource', {}).get('resourceType') == 'Patient'),
        None
    )
...
def test_patient_generator_shuffle(create_test_zip):
    ...
    assert patients1 != list(PatientGenerator(create_test_zip)) # Different from non-shuffled
...
def test_create_patient_generator_with_config_name():
    ...
    with patch('src.utils.config.Config') as mock_config:
        ...
        with patch('os.path.exists') as mock_exists, \
             patch('src.utils.patient_generator.PatientGenerator') as mock_generator:
            
            mock_exists.return_value = True
            create_patient_generator("breast_cancer/10_years")
            
            # Verify PatientGenerator was called with resolved path
            expected_path = os.path.join('data/synthetic_patients', 'breast_cancer', '10_years', 'breast_cancer_10_years.zip')
            mock_generator.assert_called_once_with(expected_path, mock_config_instance, False, None)
    ...
    with patch('src.utils.config.Config') as mock_config:
        ...
        with patch('os.path.exists') as mock_exists:
            ...
            with patch('src.utils.patient_generator.PatientGenerator') as mock_gen:
                ...
                expected_path = os.path.join('/path/to/data', 'test_type', 'test_duration', 'test_type_test_duration.zip')
                mock_gen.assert_called_with(expected_path, mock_config_instance, False, None)
