...
class TestDownloadSyntheticPatientArchives:
    ...
    def test_download_archives_request_failure(self, mock_remove, mock_exists, mock_makedirs, mock_get):
    ...
    def test_get_archive_paths(self, mock_walk):
        ...
        mock_walk.return_value = [
            ...
            ('data/synthetic_patients/mixed_cancer/10_years', [], ['mixed_cancer_10_years.zip']),
            ('data/synthetic_patients/breast_cancer', ['lifetime'], []),
            ('data/synthetic_patients/breast_cancer/lifetime', [], ['breast_cancer_lifetime.zip']),
        ]
...
class TestDownloadSyntheticPatientArchivesConcurrent:
    ...
    @patch('src.utils.data_downloader.os.makedirs')
    @patch('src.utils.data_downloader.os.path.exists')
    def test_concurrent_download_success(self, mock_exists, mock_makedirs, mock_create_task, mock_task_queue):
        ...
        mock_queue_instance.execute_tasks.return_value = [Mock(task_id="download_test.zip", success=True)]
...
class TestDownloadSingleArchive:
    ...
    @patch('src.utils.data_downloader.os.path.exists')
    @patch('src.utils.data_downloader.os.remove')
    def test_download_single_archive_success(self, mock_remove, mock_exists, mock_makedirs, mock_get):
        ...
        with patch('builtins.open', mock_open()) as mock_file:
            result = _download_single_archive("http://example.com/test.zip", "/tmp/test.zip", "test.zip")
    ...
    @patch('src.utils.data_downloader.os.path.exists')
    @patch('src.utils.data_downloader.os.remove')
    def test_download_single_archive_failure(self, mock_remove, mock_exists, mock_makedirs, mock_get):
...
class TestDownloadMultipleFiles:
    ...
    def test_download_multiple_files_success(self, mock_exists, mock_create_task, mock_task_queue):
        ...
        file_urls = {"file1.txt": "http://example.com/file1.txt", "file2.txt": "http://example.com/file2.txt"}
...
class TestDownloadSingleFile:
    ...
    def test_download_single_file_success(self, mock_makedirs, mock_get):
        ...
        with patch('builtins.open', mock_open()) as mock_file:
            result = _download_single_file("http://example.com/test.txt", "/tmp/test.txt", "test.txt")
...
class TestIntegrationScenarios:
    ...
    def test_custom_archives_config(self):
        ...
        custom_config = {
            "test_cancer": {
                "5_years": "http://example.com/test.zip"
            }
        }

        with patch('src.utils.data_downloader.requests.get') as mock_get, \
             patch('src.utils.data_downloader.os.makedirs'), \
    ...
    def test_force_download(self):
        ...
        with patch('src.utils.data_downloader.requests.get') as mock_get, \
            ...
            with patch('builtins.open', mock_open()):
                result = download_synthetic_patient_archives(force_download=True)
    ...
    def test_directory_creation(self):
        ...
        with patch('src.utils.data_downloader.requests.get') as mock_get, \
             patch('src.utils.data_downloader.os.makedirs') as mock_makedirs, \
             patch('src.utils.data_downloader.os.path.exists', return_value=False):
