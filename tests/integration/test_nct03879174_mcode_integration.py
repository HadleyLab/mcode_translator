"""
Integration test for mCODE extraction pipeline on NCT03879174.
This test validates the complete mCODE extraction and transformation process for a trial
focusing on hormone receptor positive breast cancer with ESR1 mutation.
"""
...
class TestNCT03879174MCODEIntegration:
    """Integration tests for mCODE extraction on NCT03879174"""
    ...
    def test_full_study_retrieval(self, study_data, nct_id):
        ...
        assert "Pembrolizumab And Tamoxifen" in identification_module["briefTitle"]
    ...
    def test_nlp_processing_integration(self, study_data):
        ...
        # Verify specific biomarker extraction based on trial focus
        biomarkers = features["biomarkers"]
        # This trial focuses on hormone receptor positive, HER2 negative breast cancer
        # with ESR1 mutation, so we should see these biomarkers
        assert any(bm["name"] == "ESR1" and bm["status"] == "mutated" for bm in biomarkers)
        assert any(bm["name"] == "HER2" and bm["status"] == "negative" for bm in biomarkers)
        
        # Verify demographics
        demographics = features["demographics"]
        assert demographics["gender"] == ["female"]  # Trial is for women only
    ...
    def test_mcode_mapping_integration(self, study_data):
        ...
        # Map entities to mCODE with trial context
        mapped_mcode = mcode_mapper.map_entities_to_mcode(nlp_result.entities, study_data)
        
        # Verify mCODE mapping results
        assert isinstance(mapped_mcode, list)
        
        # Should contain at least the key biomarkers for this trial
        assert len(mapped_mcode) >= 3  # ESR1 mutation, HER2 negative, hormone receptor positive
    ...
    def test_complete_pipeline_integration(self, study_data):
        ...
        # Step 4: mCODE mapping with trial context
        mcode_mapper = MCODEMappingEngine()
        mapped_mcode = mcode_mapper.map_entities_to_mcode(all_entities, study_data)
        assert isinstance(mapped_mcode, list)
