...
from src.cli.patients_fetcher import main as patients_fetcher_main, create_parser as create_patients_parser
from src.cli.patients_processor import main as patients_processor_main, create_parser as create_patients_processor_parser
from src.cli.trials_fetcher import main as trials_fetcher_main, create_parser as create_trials_parser
...
class TestPatientsFetcherCLI:
    ...
    def test_patients_fetcher_list_archives(self, mock_setup_logging, mock_create_config, mock_workflow_class):
    ...
    @patch('src.shared.cli_utils.McodeCLI.create_config')
    @patch('src.shared.cli_utils.McodeCLI.setup_logging')
    def test_patients_fetcher_fetch_patients_success(self, mock_setup_logging, mock_create_config, mock_workflow_class):
    ...
    @patch('src.shared.cli_utils.McodeCLI.create_config')
    @patch('src.shared.cli_utils.McodeCLI.setup_logging')
    def test_patients_fetcher_fetch_with_output_file(self, mock_setup_logging, mock_create_config, mock_workflow_class):
        ...
        with patch('sys.stdout', captured_output):
            ...
            parser = create_patients_parser()
            args = parser.parse_args(['--archive', 'test_archive', '--out', 'output.ndjson'])
    ...
    @patch('src.shared.cli_utils.McodeCLI.create_config')
    @patch('src.shared.cli_utils.McodeCLI.setup_logging')
    def test_patients_fetcher_fetch_failure(self, mock_setup_logging, mock_create_config, mock_workflow_class):
...
class TestTrialsFetcherCLI:
    ...
    @patch('src.shared.cli_utils.McodeCLI.create_config')
    @patch('src.shared.cli_utils.McodeCLI.setup_logging')
    def test_trials_fetcher_search_success(self, mock_setup_logging, mock_create_config, mock_workflow_class):
        ...
        with patch('sys.stdout', captured_output):
            ...
            args = parser.parse_args(['--query', 'breast cancer', '--max-results', '25'])

            trials_fetcher_main(args)

        # Check output
        output = captured_output.getvalue()
        assert "Trials search completed successfully!" in output
